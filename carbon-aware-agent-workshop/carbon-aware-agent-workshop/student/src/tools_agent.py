...
# Path to the project root (go 2 levels up from student/src)
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
DATA_FP = os.path.join(PROJECT_ROOT, "data", "mock_forecast.json")
PROFILE_FP = os.path.join(PROJECT_ROOT, "memory", "profile.json")
...
def round_15(dt: datetime) -> datetime:
    """ round off to nearest 15 minutes"""
    # Compute the number of minutes since the start of the hour
    minutes = dt.minute
    # Round to the nearest multiple of 15
    snapped = round(minutes / 15) * 15
    # Handle edge case: if rounding pushes us to 60, bump the hour
    if snapped == 60:
        dt = dt.replace(hour=dt.hour + 1, minute=0, second=0, microsecond=0)
    else:
        dt = dt.replace(minute=snapped, second=0, microsecond=0)
    return dt
...
def parse_time_tool(text_time: str, now_iso: Optional[str] = None) -> str:
    """Parse a natural-language time into ISO-8601 (rounded to nearest 15 min). 
...
def best_slot_in_window_tool(region: str, start_iso: str, window_mins: int) -> dict:
    ...
    """
    #I used GPT 4.0 to generate ideas for this function. I am responsible for the content and quality of the submitted work.
    # modified code to pick timeslot rounded off to nearest 15 min instead of floored to 15 min
    # snap_15 function replaced with round_15 function
    # parse_time_tool function also replaced snap_15 with round_15
    
    data = load_forecast()
    entries = data.get(region.upper(), [])
    if not entries:
        return {}
    start_dt = round_15(datetime.fromisoformat(start_iso))
